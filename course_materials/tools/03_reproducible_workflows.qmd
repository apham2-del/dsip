...
::: {.callout-warning title="Windows Users" collapse="true"}
...
If you want to integrate conda with PowerShell, you can try following the instructions [here](https://www.codegenes.net/blog/how-can-i-activate-a-conda-environment-from-powershell/).
:::
...
::: {.callout-note title="Including pip installed dependencies" collapse="true"}
...
1.  First, add these pip-installed packages to your `environment.yml` file. The easiest way to do this is to first run `conda env export`. This will output something like
...
2. Next, create the conda lock file as usual by running `conda lock`.
...
1.  In RStudio, open your `*.Rproj` project file in your `dsip/cancer_mortality/` (or `dsip/lab1/`) directory. If you successfully opened the project, you should see the name of the project (e.g., `cancer_mortality` or `lab1`) in the top right corner of your RStudio window.

2.  Open the quarto document and render it by clicking on the `"Render"` button at the top of the quarto document.
...
1.  Delete the `renv/library/` and `renv/staging/` directories in your project root directory (e.g., `dsip/lab1/`).

2.  For Positron and VS Code users, open your project root directory (e.g., `dsip/lab1/`) in your IDE. For RStudio users, open your `*.Rproj` project file in your project root directory (e.g., `dsip/lab1/`).

3.  Try restoring your environment by running the following command in your R console:

    ``` r
    renv::restore()
    ```

4.  Render your quarto document by running the following command in your terminal, e.g.,
...
1.  In your terminal, navigate to your project root directory (e.g., `dsip/lab1/`).

2.  Create a new conda environment from your lock file by running the following command in your terminal:
...
3.  Render your quarto document using the new conda environment by running the following command in your terminal:
...
::: {.callout-warning title="File Paths and External Files" collapse="true"}
