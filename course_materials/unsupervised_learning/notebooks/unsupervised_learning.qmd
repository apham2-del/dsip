---
title: "Introduction to Unsupervised Learning Methods"
format: html
---
...
# Generate three clusters in 3D space
...
sigma <- diag(3)  # Identity covariance (spherical clusters)

# Sample points from multivariate normal distributions
cluster1 <- MASS::mvrnorm(n, mu1, sigma)
cluster2 <- MASS::mvrnorm(n, mu2, sigma)
cluster3 <- MASS::mvrnorm(n, mu3, sigma)

# Combine data
data <- rbind(cluster1, cluster2, cluster3)
labels <- factor(rep(1:3, each = n))  # Cluster labels

# 3D Visualization
pltly <- plotly::plot_ly(
  data = as.data.frame(data) |> setNames(c("X", "Y", "Z")),
  x = ~X,
  y = ~Y,
  z = ~Z,
  color = labels,
...
plot_scatter(scores, color = labels) + 
  ggplot2::labs(color = "") +
  ggplot2::scale_color_manual(
    values = c("#66C2A4", "#F48D62", "#8E9FCA")
  )
```

**Main Takeaway**: Gaussian data is the ideal scenario for PCA.
...
# Mean vector (location of the distribution)
mu <- c(0, 0)

# Covariance matrix (controls correlation)
...
df <- data.frame(x = data[, 1], y = data[, 2], z = rnorm(n))
...
**Main Takeaway**: PCs typically group correlated variables together.
...
data <- data.table::fread(file.path(DATA_DIR, "tcga_sample.csv"))

plt <- ggplot2::ggplot(data) +
  ggplot2::aes(x = ESR1, y = GATA3) +
...
plt <- data |>
  ...
  ggwrappers::plot_histogram(
    x_str = "value", bins = 20
  ) +
  ggplot2::facet_wrap(~ name, scales = "free") +
  ggplot2::labs(x = "Gene Expression Value") +
  vthemes::theme_vmodern(size_preset = "medium")
plt
```
...
**Main Takeaway**: PCA can still be useful for non-Gaussian data as long as variance is still a meaningful measure of information.
...
t <- (3 * pi / 2) * (1 + 2 * runif(n))  # Spiral angle
x <- t * cos(t)  # X-coordinates (spiral)
y <- runif(n, min = 0, max = 25)  # Adds height variation
z <- t * sin(t)  # Z-coordinates (spiral)
...
pca_out <- prcomp(data)
scores <- as.data.frame(pca_out$x)
plot_scatter(scores, color = swiss_roll$t) + 
  ggplot2::scale_color_viridis_c()
```

**Main Takeaway**: PCA fails when the data lies on a non-linear manifold.
...
Though the PCA optimization problem may look complicated, it turns out that the
...
Autoencoders are a type of neural network architecture that can be used for
dimension reduction. Generally, an autoencoder consists of two main components: 
an encoder and a decoder. The encoder $f$ maps the input data $x$ to a lower-dimensional 
latent representation $z = f(x)$, while the decoder $g$ maps the latent representation back to the 
original data space $x' = g(z) = g(f(x))$.

![](../../../_site/images/autoencoder.png){fig-align="center" width=80%}

To learn the parameters (or weights) in the autoencoder, the neural network is trained to minimize the reconstruction error between the original data and the reconstructed data: 
\begin{align*}
\lVert x - x' \rVert^2_2 = \lVert x - g(f(x)) \rVert^2_2.
\end{align*}
In other words, the autoencoder is trained to learn a compressed representation of the data that can be used to reconstruct the original data as accurately as possible. 
